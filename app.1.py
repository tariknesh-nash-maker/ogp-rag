...
def post_filter_by_year_tuples(items, yr_min, yr_max, include_unknown=True):
    out = []
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking…"):
            ...
            for q in planned_queries[:3]:
                ...
                res = collection.query(query_embeddings=q_vec, n_results=CANDIDATES, where=where)
                if res["ids"]:
                    for md, doc, _id in zip(res["metadatas"][0], res["documents"][0], res["ids"][0]):
                        if _id not in seen_ids:
                            seen_ids.add(_id)
                            pool.append((md, doc, _id))
            ...
            if not pool:
                ...
                # Year post-filter + trim
                if effective_use_year_filter:
                    pool = post_filter_by_year_tuples(pool, eff_year_min, eff_year_max, include_unknown=include_unknown_year)
                items = pool[:TOP_K]
        ...
        if c1.button("👍 Helpful", key=f"up_{st.session_state['turn']}"):
            with open(FEEDBACK_FILE, "a", newline="", encoding="utf-8") as f:
                csv.writer(f).writerow([int(time.time()), user_msg, st.session_state["turn"],
                                        {"regions":sel_regions,"countries":effective_sel_countries,"themes":effective_sel_themes,
                                         "ATI":eff_ati,"CP":eff_cp,"PA":eff_pa,"TECH":eff_tech},
                                        st.session_state["last_ids"], True, ""])
            st.success("Thanks for the feedback!")
        if c2.button("👎 Not helpful", key=f"down_{st.session_state['turn']}"):
            note = c3.text_input("What missed the mark? (optional)", key=f"fbnote_{st.session_state['turn']}")
            with open(FEEDBACK_FILE, "a", newline="", encoding="utf-8") as f:
                csv.writer(f).writerow([int(time.time()), user_msg, st.session_state["turn"],
                                        {"regions":sel_regions,"countries":effective_sel_countries,"themes":effective_sel_themes,
                                         "ATI":eff_ati,"CP":eff_cp,"PA":eff_pa,"TECH":eff_tech},
                                        st.session_state["last_ids"], False, note])
